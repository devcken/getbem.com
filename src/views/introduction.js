export default () => (
	<div className="container introduction">
		...
		<p>소규모 브로슈어 사이트에서, 보통 스타일 체계화는 큰 관심사가 아닐 겁니다. CSS를 작성하겠지만 어쩌면 SASS를 이용할지도 모릅니다. SASS의 production 설정으로 단일 스타일시트 내에 모든 것을 컴파일한 뒤, 모듈의 모든 스타일시트를 하나의 아주 작은 패키지로 모으기 위해 합칠 겁니다.</p>
		...
		<p>또한 레거시 코드를 다루는 장기 프로젝트에 있어서도 그렇습니다.(<a href="http://webuild.envato.com/blog/how-to-scale-and-maintain-legacy-css-with-sass-and-smacss/">"Sass와 SMACSS로 Legacy CSS를 조정하고 유지하는 방법"</a>을 읽고 훌륭한 SMACSS와 BEM의 조화를 살펴보세요).</p>


		<h2>방법론</h2>

		<p>CSS codebase를 줄이고, 프로그래머 간의 협업의 체계화하며, CSS 코드를 유지보수하기 쉽게 하는데 목적을 두고 만들어진 많은 <a href="https://github.com/ikkou/awesome-css#architecture">방법론</a>들이 나오고 있습니다. 이런 방법론이 Twitter, Facebook 그리고 <a href="http://markdotto.com/2014/07/23/githubs-css/#two-bundles">Github</a>와 같은 큰 프로젝트에 있어서 분명한 것이지만, 다른 작은 프로젝트들은 상당힌 빠른 속도로 "아주 거대한 CSS 파일"을 만들어냅니다.</p>
		...
		<p>당신의 프로젝트에 어떤 방법론을 적용하더라도, 구조화된 CSS와 UI의 장점을 얻게 될 것입니다. 그것들 중 어떤 것은 덜 엄격하고 좀 더 유연하며, 다른 것들은 이해하기가 쉬워 팀 내에도 잘 전파가 될 수도 있습니다.</p>
		...
		<div className="row">
			...
			<div className="column">
				<div className="introduction__cell introduction__cell--element">
					<h3>Element</h3>
					<p>Block의 일부분이며 혼자서는 어떤 의미도 없습니다. Element는 의미적으로 자신의 block 내에 묶여야 합니다.</p>
		...
		<p>페이지 상의 하나의 개별 요소가 어떻게 BEM으로 구현되는지 살펴보도록 하죠. <a href="http://primercss.io/buttons/">GitHub</a>에서 <code>button</code>을 가져오겠습니다:</p>

		<div style={{textAlign: 'center', paddingTop: '10px', paddingBottom: '40px'}}>
			<img src="/assets/github_buttons.jpg" style={{width: '100%', maxWidth: '361px'}}/>
		</div>

		<p>보통의 경우에 사용되는 일반적인 버튼이 있으며, 그 외에 두 가지 상태의 버튼이 있습니다. 클래스 셀렉터에 의한 BEM 스타일 block 덕분에, 우리가 원하는 태그라면 무엇으로든지 block을 구현할 수 있습니다.(<code>button</code>, <code>a</code> 혹은 <code>div</code>으로도). 네이밍은 <code>block--modifier--value</code> 구문을 사용하도록 안내합니다.</p>
	...
	color: #900;
		...
		<div className="row">
			...
			<div className="column">
				<div className="introduction__cell">
					<h4>재사용성</h4>
					<p>독립적인 block들을 이전과는 다른 방법으로 구성하고, 유지하게 될 CSS 코드량을 줄이기 위해 그것들을 재사용합니다.</p>
		...
		<ul>
			<li><a href="http://blog.decaf.de/2015/06/24/why-bem-in-a-nutshell/">‘왜 BEM인가?’ 간단하게 알아보기</a></li>
			...
			<li><a href="http://www.phase2technology.com/blog/used-and-abused-css-inheritance-and-our-misuse-of-the-cascade/">사용과 오용</a> — CSS 상속 그리고 Cascade에 대한 오용</li>
		...
		<p>우리는 "기존 프로젝트를 BEM으로 이전하는 방법"이라는 글을 쓰는데 관심을 기울이고 있습니다. 그 글이 다 쓰여질 때까지, Nicole Sullivan이 작성한 프레젠테이션을 살펴보시기 바랍니다. — "<a href="http://www.youtube.com/watch?v=0NDyopLKE1w">CSS 전처리자 성능</a>". 이 프레젠테이션은 그녀가 주요 사이트에서 접해던 문제점들에 대한 좋은 설명들과 그것들을 찾고 해결한 방법들을 제공합니다.</p>
	</div>
);
